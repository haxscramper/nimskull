...
block union_pragma:
  when not defined(js): # union doesn't work yet (could use `DataFrame`s) in JS
    ## The union pragma can be applied to any object type. It means all of the
    ## object's fields are overlaid in memory. This produces a union instead of
    ## a struct in the generated C/C++ code.
    type
      Union {.union.} = object
        field1: int
        field2: array[12, int]
